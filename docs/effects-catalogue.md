...
| **Lapse** (the time and volume machine) | **new** (2026-09-22) | P1 | Two bars of memory with curves drawn over them: a **time** curve saying how far back to read and a **volume** curve saying how loud, both on a grid of the bar, plus a tone and a pan lane. Every stutter, freeze, scratch, tape stop, rewind, trance gate and groove template is one of those curves at one shape — the slope *is* the playback rate, so a 45° fall is a freeze and a steeper one is reverse. Twelve scenes switched by automation or by an octave of a keyboard, per-lane lengths, swing, and a **lookahead** chooser that buys the *future* half of the time axis with compensated latency, which the thing this is modelled on cannot do. Seventy factory presets in ten categories, six of them twelve-scene kits. **The whole design is `docs/lapse-plan.md`** and §15 of it is where building it changed the design; read it before touching anything with a bar clock in it — its §3.4 put the song's **tick** on `TransportSnapshot`, which every synced effect here had been without. |
| **Stutter / glitch** | **absorbed** into Lapse | — | A buffer that repeats what just happened *is* a time curve with a vertical drop in it, and shipping it separately would be two effects that are one machine. `docs/lapse-plan.md` §3.1. |
| **Trance gate** | **absorbed** into Lapse | — | A sixteen-step pattern of levels is Lapse's volume lane with the step tool, swing included. Same reason, same section. |
