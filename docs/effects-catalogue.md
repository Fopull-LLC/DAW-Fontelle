...
| **Frequency shifter** | **new** (2026-09-20, `Shifter`) | P2 | Shift in Hz (±5 kHz, linear — it crosses zero — with a ±20 Hz *fine* knob for the slow barber-poles), up/down/both, feedback (the barber-pole), mix. A Hilbert pair (Niemitalo's two all-pass cascades) and a quadrature oscillator. Which path carries the sample of delay, and which sign is which sideband, were measured rather than assumed — `fontelle-fx/tests/shifter.rs`. |
| **Wavefolder** (`Fold`) | **new** (2026-09-20) | P2 | Drive (0–40 dB), symmetry (a bias before the fold — the even harmonics), smooth (a crease at 0, a sine's turn at 1), output, mix. A signal past full scale folds back rather than clips, so the harmonics keep coming as the drive goes up. The distortion's *Fold* curve is one fold; this keeps going. |
| **Multiband distortion** (`Multiband`) | **new** (2026-09-20) | P1 | Two Linkwitz–Riley crossovers, a drive per band, output, mix. The low band goes through the high crossover's all-pass so the three sum flat whatever the corners. The first draft took the mid band as "what is left", which sums exactly and is 130 % of the signal an octave under the low corner — `fontelle-fx/src/multiband.rs` says why. |
...
| **Flanger** | **new** (2026-09-20) | P1 | Delay (0.1–10 ms), depth, *through-zero* switch (a second, fixed copy at the centre beside the swept one — the node's dry is then a third copy, so the pure thing is at full mix), rate (sync), feedback (±, negative for the hollow one), spread, mix. Shares the chorus's line (`lines.rs`). Wet only; half wet by default. |
| **Phaser** | **new** (2026-09-20) | P1 | Stages (2–12), centre, depth (±2 octaves), rate (sync), feedback (±), spread, mix. The same all-pass chain as the synth's `FilterModel::Phaser`, on a bus with an LFO of its own. Wet only — an all-pass alone has unit gain everywhere, and the notches are the node's sum — so half wet by default. |
| **Hyper** | **new** (2026-09-20) | P1 | Voices (1–4), detune (cents), spread, window (5–50 ms), mix. Unison as an effect: each copy a delay line read at a steady rate other than one, two heads crossfaded over the window. The crossfade's two heads comb where their phases differ, which is the granular shifter's own sound and why a copy's line reads a little under a quarter. Half wet. |
...
| **DisgustingBeat** (the time and volume machine) | **new** (2026-09-22) | P1 | Two bars of memory with curves drawn over them: a **time** curve saying how far back to read and a **volume** curve saying how loud, both on a grid of the bar, plus a tone and a pan lane. Every stutter, freeze, scratch, tape stop, rewind, trance gate and groove template is one of those curves at one shape — the slope *is* the playback rate, so a 45° fall is a freeze and a steeper one is reverse. Twelve scenes switched by automation or by an octave of a keyboard, per-lane lengths, swing, and a **lookahead** chooser that buys the *future* half of the time axis with compensated latency, which the thing this is modelled on cannot do. Seventy factory presets in ten categories, six of them twelve-scene kits. **The whole design is `docs/disgusting-beat-plan.md`** and §15 of it is where building it changed the design; read it before touching anything with a bar clock in it — its §3.4 put the song's **tick** on `TransportSnapshot`, which every synced effect here had been without. |
| **Stutter / glitch** | **absorbed** into DisgustingBeat | — | A buffer that repeats what just happened *is* a time curve with a vertical drop in it, and shipping it separately would be two effects that are one machine. `docs/disgusting-beat-plan.md` §3.1. |
| **Trance gate** | **absorbed** into DisgustingBeat | — | A sixteen-step pattern of levels is DisgustingBeat's volume lane with the step tool, swing included. Same reason, same section. |

### 2.5 Stereo, utility and metering

| Effect | Status | Priority | The family it has to be |
|---|---|---|---|
| **Width** | **new** (2026-09-20) | P1 | Width (0–200 %), mono below (the side high-passed), mid gain, side gain, mix. The utility has a width and a mono-maker; this is the one reached for as an effect, with the two gains, and the one a patch's own chain offers (`PATCH_FX_KINDS`). |
