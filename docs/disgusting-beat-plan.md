# DisgustingBeat: the time and volume machine (2026-09-22)
...
**DisgustingBeat is a bar of memory with a curve drawn over it.** The insert
keeps the last two bars of everything that passed through it. A **time curve**
says, for each point in the bar, how far back in that memory to read; a
**volume curve** says how loud to play it. Both are drawn, in the window, over
a grid of the bar. Everything the original is famous for — the stutter, the
hold, the scratch, the tape stop, the half-time — is one of those two curves at
a particular shape, and nothing else.
...
1. A fresh DisgustingBeat is **a wire, sample for sample** — the time curve flat at
...
**Settled: `DisgustingBeat`** — Ty's, on 2026-09-22, before the first release
that carries it. `EffectKind::DisgustingBeat`, `label()` →
`"DisgustingBeat"`, slug `fx-disgusting-beat`.

The rest of this document was written under the working name **Lapse** and was
renamed mechanically along with the tree, so where the prose reads oddly —
"a DisgustingBeat", "the DisgustingBeat's window" — that is why. The working
name's case for itself was: one word, in the family of *Soften*, *Width*,
*Hyper*, *Fold*, *Tune*; about time; and free of the other program's
trademark. The alternatives considered were *Flopbeat* (matches Flopsynth),
*Rewind*, *Grab* and *Loophole*.

The name that won is the joke the plugin is named after, told back. It meets
the one hard requirement — it is not the other program's trademark — and it
says what the thing is to anybody who has heard of the original.

It was **cheap on the day it was decided and expensive on any day after**: a
slug is a folder name and therefore INVARIANT 7's, so renaming it once a
release exists moves everybody's presets. That is the whole reason the
question was asked before the version was bumped rather than after, and the
whole reason this section now reads as history rather than as a question.

---

## 3. Where it sits: the five decisions

### 3.1 It is an insert, `EffectKind::DisgustingBeat`
...
delay's output belongs *under* the track; DisgustingBeat's output *replaces*
it. It opens fully wet, like every other processor. (A hold at 50 % mix is the
hold flamming against the live signal. That is a real sound and somebody will
use it; it is not a default.)
...
- **`DisgustingBeatConfig`** is the knobs — depths, smoothing, quality, lookahead,
  which scene is playing, mix. `Copy`, small, automatable, and on the existing
  triple buffer with every other insert's config.
- **`EffectSlot::disgusting_beat: Option<Box<DisgustingBeatBank>>`** is the drawn material.
  Boxed, so every other insert pays one null pointer for it; `Some` on every
  DisgustingBeat from `EffectSlot::new` onwards, `None` everywhere else, and
...
This is where DisgustingBeat differs from the notepad, and it is the one
genuinely new piece of engine plumbing. A pad's words never cross; a curve
must, and it must cross *while somebody is dragging it*, because a curve editor
whose sound arrives on the next graph rebuild is a curve editor nobody can use.
A graph rebuild costs a patch deserialisation per channel
(`effect_channel.rs`'s own module comment) and is not available sixty times a
second.

So: **`crates/fontelle-engine/src/disgusting_beat_channel.rs`**, the sibling of
`effect_channel.rs`, written to the same shape and for the same reason.

```rust
pub fn disgusting_beat_channel(grid: DisgustingBeatGrid) -> (DisgustingBeatControls, DisgustingBeatSource);
impl DisgustingBeatControls { pub fn publish(&mut self, grid: DisgustingBeatGrid); }
impl DisgustingBeatSource   { /** RT. */ pub fn current(&mut self) -> &DisgustingBeatGrid; }
```
...
- **`current` returns a reference.** `EffectSource::current` returns
  ...
  `DisgustingBeatGrid` is 50 KB and copying it per block per insert would be 19 MB/s of
  memcpy for nothing. `triple_buffer::Output::read` already hands back a
  reference; take it.
- **`DisgustingBeatGrid` is a realised form, not the document's.** The document stores
  `Vec<DisgustingBeatPoint>` because that is what an editor and an undo stack want; the
...
Attached to the node with `EffectNode::with_disgusting_beat(source)`, beside
...
**What this unblocks besides DisgustingBeat**: the catalogue's tremolo, vibrato
and every future synced LFO can start *in phase with the bar* instead of
wherever the graph was last reset. That is a long-standing quiet wrongness in
this program and this is the fix for it.
...
**DisgustingBeat is that editor.** So:
...
`crates/fontelle-fx/src/disgusting_beat.rs`. One struct, `DisgustingBeat`, with
`new`, `prepare` (the only place that allocates), `reset`, and
...
Three clock modes (`DisgustingBeatSync`):
...
A ring of `DISGUSTING_BEAT_MEMORY_SECONDS = 12.0` seconds per channel, sized in
...
a rule, not an accident, and `fontelle-fx/tests/disgusting_beat.rs` holds it.
...
(`fontelle-fx/tests/disgusting_beat.rs`'s alias case, the shape `synth_alias.rs`
...
A fresh DisgustingBeat is a wire (rule 2): the depths are at full but scene 1's
lanes are flat — time at zero, volume at unity, tone and pan off — so it passes
the signal through bit for bit until somebody draws something. That is the
right way round: the depth knobs are there to dial *back* a preset, and a
preset is what somebody will load ten seconds after adding it.

---

## 6. The document, and the edits

`crates/fontelle-types/src/disgusting_beat.rs`:

```rust
pub struct DisgustingBeatBank  { pub scenes: Vec<DisgustingBeatScene> }          // twelve
pub struct DisgustingBeatScene { pub name: String, pub lanes: [DisgustingBeatLane; 4] }
pub struct DisgustingBeatLane  { pub points: Vec<DisgustingBeatPoint>, pub length: Tick, pub on: bool }
pub struct DisgustingBeatPoint { pub at: f64, pub value: f64, pub curve: CurveShape, pub tension: f32 }
pub enum   DisgustingBeatLaneKind { Time, Volume, Tone, Pan }
```
...
**The edits are an algebra.** `DisgustingBeatEdit::apply` does one thing and
returns **the inverse of what it just did**, so `EditLapse` in `fontelle-model`
has nothing to work out and no second copy of the rules — the shape
...
pub enum DisgustingBeatEdit {
    AddPoint   { scene: usize, lane: usize, point: DisgustingBeatPoint },
    ...
    FillLane   { scene: usize, lane: usize, shape: DisgustingBeatFill },   // §7.4's shape menu
...
`Session` publishes the realised `DisgustingBeatGrid` on the channel after
every applied edit (one `publish` per command, not per point), and on a
structural rebuild the channel is opened fresh from the document, exactly as
...
**A Flopsynth window**, as asked: `fontelle-ui/src/canvas/disgusting_beat.rs` +
`draw_disgusting_beat*` in `fontelle-ui/src/render/`, reusing `draw_flop_knob`,
...
```
┌──────────────────────────────────────────────────────────────────────┐
│  DISGUSTING_BEAT            ‹ preset bar ›            A/B  INIT  ⟲ ⟳    75 %▾  │  header
├──────────────────────────────────────────────────────────────────────┤
...
`DisgustingBeatTap` / `DisgustingBeatFrame` in
`crates/fontelle-engine/src/disgusting_beat_tap.rs`, the sibling of
`tune_tap.rs`, published per block and read per frame:
...
### 7.6 Tests for the window (`fontelle-ui/tests/disgusting_beat.rs`)

- `the_whole_disgusting_beat_window_fits_the_window_it_opens_at` (1180×760, real view,
...
touches them has to name all of them. With DisgustingBeat there are **five**,
and the notepad's three-list omission (`docs/handoff.md` §1) is a whole class
of bug waiting to happen again. Answer every one of these:
...
Notepad(NotepadView), DisgustingBeat(DisgustingBeatView) }` first.** It makes
the class of bug unrepresentable — a `match` the compiler checks instead of
five `is_none()` calls somebody has to remember — and it is about forty
mechanical sites in `app.rs`. Do it as its own commit, with the suite green
...
and a DisgustingBeat preset with no curve in it is not a preset. Add a **new
variant** — `PresetPayload::DisgustingBeat { config: DisgustingBeatConfig,
bank: DisgustingBeatBank }` — rather than adding a field to the existing one: a
newtype variant that grew a field would change the JSON shape of every effect
preset file in the factory tree *and* in every user's bank. A new variant
leaves all of them untouched and costs one `match` arm in each of the handful
of places that read a payload. `PRESET_FORMAT_VERSION` does **not** move;
nothing old becomes unreadable.

**Sixty-four presets in nine categories**, plus six kits. Categories are a free
string on `Preset` and the browser groups by them; effects have all used
`"Factory"` until now because their presets are points on one control surface.
DisgustingBeat's are not — a scratch and a sidechain pump have nothing to do
with each other — so it is the first effect with real categories, and that is
...
`fontelle-types/src/disgusting_beat_presets.rs` beside `effect_presets.rs`. Every preset
...
tension, §3.5). `disgusting_beat.rs`: the bank types, `DisgustingBeatGrid` +
`From<&DisgustingBeatBank>`, `DisgustingBeatEdit::apply`,
`DisgustingBeatConfig` with `new()`, `get`/`set` by id,
`DISGUSTING_BEAT_PARAMS`, `DISGUSTING_BEAT_SECTIONS`, the five enums with
`label`/`ALL`/position tables, `DisgustingBeatPreset::ALL` + `from_preset`.
`effect.rs`: the `EffectKind` variant, `label`, `ALL`, `takes_notes`, the
`EffectConfig` arm. `preset.rs`: the `fx-disgusting-beat` slug and the
`PresetPayload::DisgustingBeat` variant. `event.rs`: `TempoSegment`, `tick_at`,
`beats_per_bar` (§3.4).
...
**`fontelle-fx`** — `disgusting_beat.rs`: `DisgustingBeat::new`, `prepare`,
`reset`, `process`, `latency_samples`, `tap_frame`. Owns the ring, the two read
heads, the four lane cursors, the SVF pair and the envelope buckets. `lib.rs`
exports it.
...
`disgusting_beat_channel.rs` and `disgusting_beat_tap.rs` (§3.3, §7.2).
`nodes.rs`: the `EffectState::DisgustingBeat` arm (new/prepare/process/reset),
`MusicalTime` built from `ctx.transport`, `with_disgusting_beat`,
`with_disgusting_beat_tap`, the second arm of `insert_latency_samples` and the
`max_insert_latency_samples` the dry line is sized from. `lib.rs` exports.

**`fontelle-model`** — `mixer.rs`: `EffectSlot.disgusting_beat`. `commands.rs`:
...
**`fontelle-app`** — `disgusting_beat.rs`: `describe(config, bank, tap) ->
DisgustingBeatView` and `shape_of`, the layer allowed to see both halves
(INVARIANT 4), sibling of `tune.rs`. `realise.rs`: the channel, the tap,
`with_disgusting_beat`. `session.rs`: the host methods, the
`disgusting_beat_taps` map, the publish after every edit, the re-prepare on a
lookahead change.

**`fontelle-ui`** — §7.

**`xtask`** — `presets.rs`: the `DisgustingBeatPreset` loop and the kits.
...
### 10.2 `fontelle-fx/tests/disgusting_beat.rs`
...
### 10.3 `fontelle-types/tests/disgusting_beat.rs` and `parameters.rs`
...
- `tension_zero_is_exactly_todays_eased` — the automation lane may not change
  shape.
- `the_slug_is_fx_disgusting_beat` (the frozen-strings table).

### 10.4 `fontelle-engine/tests/effects.rs` (additions) and `disgusting_beat_node.rs`

- The generic sweep covers the kind the moment it is in `ALL` — including the
  440 Hz-tone-through-every-effect case, which a flat DisgustingBeat passes by
  definition.
...
- `a_disgusting_beat_slot_opens_with_twelve_flat_scenes_and_a_notepad_slot_does_not`.
- `a_project_written_before_disgusting_beat_opens_and_writes_back_unchanged`.

### 10.6 `fontelle-app/tests/disgusting_beat_editor.rs`, `preset_bank.rs`

§8's four gates, plus the preset bar's dirty `*` and the undo entry per preset
click that every device already has.

### 10.7 `fontelle-ui/tests/disgusting_beat.rs`
...
- **Memory**: 4.6 MB stereo at 48 kHz per insert (§4.2). Ten DisgustingBeats is 46 MB.
...
- **The envelope buckets**: O(1) a sample, but it is a scatter write into a
  ...
  pass.
- **The `DisgustingBeatGrid` publish**: 50 KB per edit. At a drag's sixty edits a
...
**Phase 1 — the machine (two days).** §4 in `fontelle-fx` against a hand-built
`DisgustingBeatGrid`, with §10.2 written first. Everything measured here is
...
algebra, the command, the realise layer. §10.3–10.5. At the end of this phase a
DisgustingBeat can be added to a strip, automated and undone — with no window.
...
- **A per-voice or per-patch DisgustingBeat.** Not in `PATCH_FX_KINDS`: an instrument's
...
1. ~~**The name.**~~ **Answered 2026-09-22: `DisgustingBeat`** (§2), before
   the release that would have frozen it.
...
4. **Categories in the preset browser for one effect** (§8). Every other
   effect uses `"Factory"`; DisgustingBeat would be the first with nine categories, and
...
7. **DisgustingBeat is processed in `EffectNode::process`**, not in
...
11. **A preset's name is a file name.** `1/8 Roll` failed the export with
    ...
    for every device.
12. **A track chain carries a DisgustingBeat's curves**, where it does not carry a
    notepad's words: a pad's pages are a document somebody wrote, and a
    DisgustingBeat's curves are the effect itself.
...
**What Phase 6 closed** — the list above, and three things it found on the
way:

18. **Five document edits had no gesture at all.** `SetLength`, `ClearLane`,
    `SetCurve`, `CopyScene` and `RenameScene` were written, undoable, tested
    and unreachable; so was `set_disgusting_beat_zoom`, which was clamped,
    carried in the view and called from nowhere. A plan that lists what the
    document can
    do is not a list of what the window can do, and the gap between them is
    invisible to every test in this repository. The lane header carries the
    length and a *clear*; the toolbar carries the reach; right-click opens the
    shape menu on a point; a scene chip renames on right-click and copies on a
    drag.
19. **The canopy measured the head against what was in the memory**, not
    against the memory. The ring is always twelve seconds of columns with the
    unwritten ones empty on the left, so a read a quarter of a second back is
    a quarter of a second from the right edge — dividing by `filled_seconds`
    put the head halfway across a picture of nothing for the first seconds
    after a start. Found by drawing the trail beside the head and watching the
    two disagree; `canvas::canopy_head` is now the one answer both use.
20. **The read head's history is the memory's own picture.** Not a second
    plot: the trail is one number per bucket — how far behind the write head
    the read was when that bucket was written — and the canopy *lights the
    column it names*. A freeze burns one column, a stutter burns three, a
    reverse burns a swath, a wire lights everything it has played (brightest
    at now, because a newer moment burns over an older one), and the part of
    the memory a curve never reached stays dark. It costs 512 relaxed atomic
    stores a block and answers "what is this curve doing" without a word.

The window also carries **one sentence for whatever is under the pointer**
(`canvas::disgusting_beat_tip`), in the canopy beside the picture rather than
...
which is the only one of the six that changes the sound.
