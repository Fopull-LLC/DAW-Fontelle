//! DisgustingBeat's factory bank: sixty-four rows in nine categories, and six
//! kits (`docs/disgusting-beat-plan.md` §8).
//!
...
impl Scene {
    ...
    const fn beat(time: &'static [P], volume: &'static [P]) -> Self {
        Self {
            time,
            volume,
            length: DisgustingBeatLength::Beat,
        }
    }
...
fn points_of(points: &[P]) -> Vec<DisgustingBeatPoint> {
    points
        .iter()
        .map(|P(at, value, shape)| {
            DisgustingBeatPoint::new(*at as f64 / 1000.0, *value as f64 / 1000.0, shape.shape())
        })
        .collect()
}
...
impl DisgustingBeatPreset {
    /// Every factory row, in the order the browser lists them.
    pub const ALL: &'static [DisgustingBeatPreset] = ROWS;

    /// The bank and the knobs this row is.
    pub fn build(&self) -> (DisgustingBeatConfig, DisgustingBeatBank) {
        let mut bank = DisgustingBeatBank::new();
        match self.recipe {
            Recipe::One(scene) => draw(&mut bank.scenes[0], &scene),
            Recipe::Kit(scenes) => {
                for (index, scene) in scenes.iter().take(DISGUSTING_BEAT_SCENES).enumerate() {
...
fn draw(scene: &mut DisgustingBeatScene, recipe: &Scene) {
    let time = scene
    ...
    time.tidy(DisgustingBeatLaneKind::Time);
    ...
    volume.tidy(DisgustingBeatLaneKind::Volume);
}
...
static ROWS: &[DisgustingBeatPreset] = &[
    // ------------------------------------------------------------ Stutter
    row(
        "Quarter Roll",
        "Stutter",
    ...
    // project (`docs/disgusting-beat-plan.md` §8).
    row(
        "Swing 16",
        "Groove",
    ...
    // channel (`docs/disgusting-beat-plan.md` §4.7).
    kit(
        "Roll Kit",
