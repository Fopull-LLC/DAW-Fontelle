//! Lapse: two bars of memory with curves drawn over them
//! (`docs/lapse-plan.md`).
//!
...
//!
//! [`LapseConfig`] is the knobs. The drawn material is a [`LapseBank`] on the
//! slot beside it, exactly as a notepad's pages and a hosted plugin's state
//! are, and for the same reason: an [`EffectConfig`](crate::EffectConfig) is
//! `Copy`, fixed-size and handed to the audio thread every block, and twelve
//! scenes of four lanes is 48 KB.
//!
...
//! [`LapseGrid`] is for: the document's shape is a `Vec` of points, because
//! that is what an editor and an undo stack want, and the grid is the same
//! thing as a fixed-size POD that a triple buffer can carry with no
//! allocation on either end.
//!
//! # Why a lane's values are in natural units and not 0..1
//!
//! `docs/lapse-plan.md` §6 said normalised, and building it said otherwise.
//! The **time** lane's value has to have an exact zero — a fresh Lapse is a
//! wire, and "no offset" may not be a number like 0.6667 that a preset can
//! miss by a rounding error. And its scale has to be *the lane's own length*,
//! because that is what makes a freeze a freeze at any length: an offset
//! falling by one lane-length over one lane is one beat per beat whatever the
//! lane is. So the time lane's value is **in lane-lengths**, zero is now, −1
//! is a whole lane back, and the freeze is the diagonal.
//!
...
pub const LAPSE_SCENES: usize = 12;

/// Time, volume, tone, pan — see [`LapseLaneKind`].
pub const LAPSE_LANES: usize = 4;

/// The most points one lane can hold.
///
/// Enforced in the document rather than in the window, because [`LapseGrid`]
/// is fixed-size and a silent truncation would be a curve that plays
/// differently from the one on screen. Sixty-four is four bars of sixteenths
/// with a point to spare.
pub const LAPSE_POINTS: usize = 64;

/// Seconds of audio a Lapse remembers, per channel.
///
...
pub const LAPSE_MEMORY_SECONDS: f32 = 12.0;

/// The four things a Lapse scene draws.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, serde::Serialize, serde::Deserialize)]
pub enum LapseLaneKind {
...
impl LapseLaneKind {
    pub const ALL: [Self; LAPSE_LANES] = [Self::Time, Self::Volume, Self::Tone, Self::Pan];
    ...
    /// Lapse is a wire and its window is two lanes rather than four.
...
pub struct LapsePoint {
    /// 0..1 across the lane's own length.
    pub at: f64,
    /// In the lane kind's own unit — see [`LapseLaneKind`].
    pub value: f64,
...
impl LapseLane {
    ...
    pub fn flat(kind: LapseLaneKind) -> Self {
        Self {
    ...
    pub fn tidy(&mut self, kind: LapseLaneKind) {
        ...
        self.points.truncate(LAPSE_POINTS);
        if self.points.is_empty() {
            self.points
                .push(LapsePoint::new(0.0, kind.neutral(), CurveShape::Linear));
        }
    }
    ...
    /// disagree.
    pub fn value_at(&self, phase: f64, kind: LapseLaneKind) -> f64 {
        if !self.on {
            return kind.neutral();
        }
    ...
    pub fn is_flat(&self, kind: LapseLaneKind) -> bool {
        self.points
            .iter()
    ...
    pub fn flat() -> Self {
        Self {
            ...
            lanes: LapseLaneKind::ALL
                .iter()
                .map(|k| LapseLane::flat(*k))
                .collect(),
        }
    }

    pub fn lane(&self, kind: LapseLaneKind) -> Option<&LapseLane> {
        self.lanes.get(kind.index())
    }

    pub fn lane_mut(&mut self, kind: LapseLaneKind) -> Option<&mut LapseLane> {
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct LapseBank {
    pub scenes: Vec<LapseScene>,
}

impl LapseBank {
    /// Twelve flat scenes: a Lapse somebody has just added and not yet drawn
    /// in, which is a wire.
    pub fn new() -> Self {
        Self {
            scenes: (0..LAPSE_SCENES).map(|_| LapseScene::flat()).collect(),
        }
    }
    ...
    pub fn fill(&mut self) {
        self.scenes.truncate(LAPSE_SCENES);
        while self.scenes.len() < LAPSE_SCENES {
            self.scenes.push(LapseScene::flat());
        }
        for scene in &mut self.scenes {
            scene.lanes.truncate(LAPSE_LANES);
            while scene.lanes.len() < LAPSE_LANES {
                let kind = LapseLaneKind::ALL[scene.lanes.len()];
                scene.lanes.push(LapseLane::flat(kind));
            }
            for (index, lane) in scene.lanes.iter_mut().enumerate() {
                lane.tidy(LapseLaneKind::ALL[index]);
            }
        }
    }

    pub fn scene(&self, index: usize) -> Option<&LapseScene> {
        self.scenes.get(index)
    }
}

impl Default for LapseBank {
    fn default() -> Self {
        Self::new()
    }
}
...
/// One thing the editor does to a bank (`docs/lapse-plan.md` §6).
///
/// [`LapseBank::apply`] does it and hands back **the inverse of what it just
/// did**, so `EditLapse` in `fontelle-model` has nothing to work out and no
/// second copy of these rules — the shape [`NotepadEdit`](crate::NotepadEdit)
/// and [`WavetableEdit`](crate::WavetableEdit) both take, for the same
/// reason.
...
impl LapseEdit {
    ...
    pub fn memory_cost(&self) -> usize {
        std::mem::size_of::<Self>()
            + match self {
                Self::SetLane { points, .. } => points.len() * std::mem::size_of::<LapsePoint>(),
                Self::SetScene { value, .. } => value
                    .lanes
                    .iter()
                    .map(|lane| lane.points.len() * std::mem::size_of::<LapsePoint>())
                    .sum(),
...
impl LapseBank {
    ...
    pub fn apply(&mut self, edit: &LapseEdit) -> Option<LapseEdit> {
        match edit {
            LapseEdit::AddPoint { scene, lane, point } => {
                let kind = *LapseLaneKind::ALL.get(*lane)?;
                let target = self.scenes.get_mut(*scene)?.lanes.get_mut(*lane)?;
                if target.points.len() >= LAPSE_POINTS {
                    return None;
                }
            ...
            LapseEdit::RemovePoint { scene, lane, index } => {
                let kind = *LapseLaneKind::ALL.get(*lane)?;
                let target = self.scenes.get_mut(*scene)?.lanes.get_mut(*lane)?;
                if target.points.len() <= 1 || *index >= target.points.len() {
                    // A lane always has a point. See `LapseLane`.
                    return None;
                }
                let before = target.points.clone();
            ...
            LapseEdit::Draw {
                ...
                target.points.push(LapsePoint::new(
                    low.0,
                    low.1,
                    before
                        .iter()
                ...
                if (high.0 - low.0).abs() > 1e-9 {
                    target
                        .points
                        .push(LapsePoint::new(high.0, high.1, CurveShape::Linear));
                }
                target.tidy(kind);
                if target.points == before {
                    return None;
            ...
            LapseEdit::SetLength {
                ...
                Some(LapseEdit::SetLength {
                    scene: *scene,
                    lane: *lane,
                    length: was,
                })
            }
            LapseEdit::SetLaneOn { scene, lane, on } => {
                let target = self.scenes.get_mut(*scene)?.lanes.get_mut(*lane)?;
                if target.on == *on {
                    return None;
                }
                target.on = *on;
                Some(LapseEdit::SetLaneOn {
                    scene: *scene,
                    lane: *lane,
                    on: !*on,
                })
            }
            LapseEdit::ClearLane { scene, lane } => {
                let kind = *LapseLaneKind::ALL.get(*lane)?;
                let target = self.scenes.get_mut(*scene)?.lanes.get_mut(*lane)?;
                let before = target.points.clone();
                target.points = vec![LapsePoint::new(0.0, kind.neutral(), CurveShape::Linear)];
                if target.points == before {
                    return None;
                }
                Some(LapseEdit::SetLane {
            ...
            LapseEdit::CopyScene { from, to } => {
                ...
                *self.scenes.get_mut(*to)? = source;
                Some(LapseEdit::SetScene {
                    scene: *to,
                    value: Box::new(before),
                })
            }
            LapseEdit::SetScene { scene, value } => {
...
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct RtLane {
    pub points: [RtPoint; LAPSE_POINTS],
    pub len: u8,
    pub length: LapseLength,
    pub on: bool,
}
...
impl RtLane {
    pub const fn empty() -> Self {
        Self {
            points: [RtPoint::NOTHING; LAPSE_POINTS],
            len: 0,
            length: LapseLength::Bar,
            on: false,
        }
...
/// is on.
///
/// The document holds [`LapsePoint`] and the audio thread holds [`RtPoint`] —
/// the same four numbers at two precisions — and [`curve_at`] is written once
/// over both.
...
impl CurvePoint for LapsePoint {
    fn at(&self) -> f64 {
        self.at
    }
    fn value(&self) -> f64 {
...
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct RtScene {
    pub lanes: [RtLane; LAPSE_LANES],
}

impl RtScene {
    pub const fn empty() -> Self {
        Self {
            lanes: [RtLane::empty(); LAPSE_LANES],
        }
    }
}

/// The whole bank, realised: what crosses to the audio thread on its own
/// channel (`docs/lapse-plan.md` §3.3).
///
...
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct LapseGrid {
    pub scenes: [RtScene; LAPSE_SCENES],
}
...
impl LapseGrid {
    ...
    pub const fn empty() -> Self {
        Self {
            scenes: [RtScene::empty(); LAPSE_SCENES],
        }
    }

    pub fn scene(&self, index: usize) -> &RtScene {
        &self.scenes[index.min(LAPSE_SCENES - 1)]
    }

    pub fn lane(&self, scene: usize, kind: LapseLaneKind) -> &RtLane {
        &self.scene(scene).lanes[kind.index()]
    }
}

impl Default for LapseGrid {
    fn default() -> Self {
        Self::empty()
    }
}

impl From<&LapseBank> for LapseGrid {
    ...
    fn from(bank: &LapseBank) -> Self {
        let mut grid = Self::empty();
        for (scene_index, scene) in bank.scenes.iter().take(LAPSE_SCENES).enumerate() {
            for (lane_index, lane) in scene.lanes.iter().take(LAPSE_LANES).enumerate() {
                let out = &mut grid.scenes[scene_index].lanes[lane_index];
                out.length = lane.length;
                out.on = lane.on;
                let mut count = 0;
                for point in lane.points.iter().take(LAPSE_POINTS) {
...
/// The knobs. The curves are a [`LapseBank`] beside it.
#[derive(Debug, Clone, Copy, PartialEq, serde::Serialize, serde::Deserialize)]
...
impl LapseConfig {
    /// A fresh Lapse. The depths are at full and the bank beside it is flat,
    /// so it is a wire until somebody draws something — which is the right
    /// way round: the depth knobs are there to dial a *preset* back, and a
    /// preset is what somebody will load ten seconds after adding one.
    pub fn new() -> Self {
        Self {
            scene: 0,
            rate: LapseRate::One,
            sync: LapseSync::Song,
            swing: 0.0,
            notes: LapseNotes::Select,
            time: 1.0,
            smooth_ms: 12.0,
            quality: LapseQuality::Normal,
            look: LapseLook::Off,
            volume: 1.0,
            tone: 0.0,
            tone_mode: LapseTone::LowHigh,
            tone_range: 3.0,
            pan: 0.0,
            output_db: 0.0,
            mix: 1.0,
    ...
    pub(crate) fn get(&self, id: &str) -> Option<f32> {
        Some(match id {
            ...
            "rate" => position_of(&LapseRate::ALL, self.rate),
            "sync" => position_of(&LapseSync::ALL, self.sync),
            "swing" => self.swing * 100.0,
            "notes" => position_of(&LapseNotes::ALL, self.notes),
            "time" => self.time * 100.0,
            "smooth" => self.smooth_ms,
            "quality" => position_of(&LapseQuality::ALL, self.quality),
            "look" => position_of(&LapseLook::ALL, self.look),
            "volume" => self.volume * 100.0,
            "tone" => self.tone * 100.0,
            "tone_mode" => position_of(&LapseTone::ALL, self.tone_mode),
    ...
    pub(crate) fn set(&mut self, id: &str, value: f32) {
        match id {
            ...
            "rate" => self.rate = at_position(&LapseRate::ALL, value),
            "sync" => self.sync = at_position(&LapseSync::ALL, value),
            "swing" => self.swing = value / 100.0,
            "notes" => self.notes = at_position(&LapseNotes::ALL, value),
            "time" => self.time = value / 100.0,
            "smooth" => self.smooth_ms = value,
            "quality" => self.quality = at_position(&LapseQuality::ALL, value),
            "look" => self.look = at_position(&LapseLook::ALL, value),
            "volume" => self.volume = value / 100.0,
            "tone" => self.tone = value / 100.0,
            "tone_mode" => self.tone_mode = at_position(&LapseTone::ALL, value),
...
const SCENE_NAMES: [&str; LAPSE_SCENES] = [
...
pub(crate) static LAPSE_PARAMS: [crate::ParamSpec; 16] = with_mix(&LAPSE_OWN_PARAMS, ALL_WET);

/// Lapse replaces the signal rather than sitting under it — a hold at half
/// mix is the hold flamming against the live sound, which is a real thing to
/// want and not a default.
const ALL_WET: f32 = 100.0;

pub(crate) static LAPSE_SECTIONS: [crate::ParamSection; 6] = [
    crate::ParamSection {
        name: "Pattern",
        count: 5,
    },
...
static LAPSE_OWN_PARAMS: [crate::ParamSpec; 15] = [
    crate::ParamSpec {
        id: "scene",
        name: "Scene",
        min: 1.0,
        max: LAPSE_SCENES as f32,
        default: 1.0,
        unit: crate::Unit::None,
        taper: crate::Taper::Stepped(LAPSE_SCENES as u32),
...
pub struct MusicalTime {
    ...
    /// a Lapse runs its own clock at `bpm` while this is false and re-locks
    /// to the song on the first rolling block.
    pub rolling: bool,
}
...
pub fn lane_ticks(length: LapseLength, rate: LapseRate, beats_per_bar: u32) -> f64 {
