...
/// The words a preset may be tagged with (`docs/flopsynth-next.md` §5.1):
/// one controlled vocabulary, so a browser's "every bass with unison"
/// finds every one and a typo tags nothing. The first fourteen are the
/// shelves' own words; the rest are what a row *is* — its sources, what
/// it does — most of which `presets::tags_of` derives from the patch.
pub const TAGS: &[&str] = &[
    // The shelves.
    "bass",
    "lead",
