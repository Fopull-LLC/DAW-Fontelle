...
/// One key, or a few that mean the same thing, and what it does.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Keybind {
    /// As written on the sheet: `Ctrl+Shift+Z`, `Space`, `+ / -`. A slash or
...
const fn bind(keys: &'static str, does: &'static str) -> Keybind {
    Keybind { keys, does }
}
...
pub const KEYBINDS_HINT: &str = "Esc or F1 closes this page. Letters work in whichever panel is showing; Ctrl keys work everywhere.";
...
pub const KEYBIND_SECTIONS: &[KeybindSection] = &[
    KeybindSection {
        ...
        binds: &[
            bind(
                "Space",
                "Play from the marker; again to stop and come back to it",
            ),
            bind("Home", "Stop, and go to the start of the song"),
            bind("Ctrl+M", "Metronome on or off"),
            bind("Ctrl+L", "Song or Clip mode (with no notes selected)"),
        ],
    },
    KeybindSection {
        title: "Project",
        binds: &[
            bind("Ctrl+S", "Save"),
            bind("Ctrl+Z", "Undo"),
            bind("Ctrl+Shift+Z / Ctrl+Y", "Redo"),
            bind("Ctrl+E", "Export the song as a WAV"),
            bind("Ctrl+Shift+E", "Export the song as a MIDI file"),
            bind("F1", "This page"),
            bind(
                "Esc",
    ...
    KeybindSection {
        ...
        binds: &[
            bind("1", "Show the piano roll"),
            bind("2", "Show the mixer"),
            bind("F", "Rack: Instruments or Prefabs"),
            bind("Tab", "Swap which is taller, the arrangement or the editor"),
            bind("Ctrl+T", "Show or hide the arrangement"),
            bind("T", "The Tools panel"),
            bind("Ctrl+F", "Search the browser"),
            bind("+ / -", "Zoom in or out, about the pointer"),
        ],
    },
    KeybindSection {
        title: "Tools",
        binds: &[
            bind("P", "Draw"),
            bind("B", "Paint (piano roll)"),
            bind("E", "Select"),
            bind("D", "Delete (piano roll)"),
            bind("C", "Slice"),
            bind("S", "Snap grid (piano roll) / Stretch (arrangement)"),
            bind("A", "Slide notes on or off"),
            bind("L", "Next property lane: velocity, pan, pitch…"),
            bind("G", "Ghost notes from other clips"),
        ],
    },
    KeybindSection {
        title: "Editing notes and clips",
        binds: &[
            bind("Ctrl+A", "Select every note"),
            bind("Ctrl+C / Ctrl+X / Ctrl+V", "Copy, cut, paste"),
            bind("Ctrl+B / Ctrl+D", "Duplicate the selection after itself"),
            bind("Delete / Backspace", "Delete the selection"),
            bind("Ctrl+Shift+M", "Mute the selected clips"),
            bind("\u{2190} \u{2192}", "Move one snap step"),
            bind(
                "\u{2191} \u{2193}",
                "Move a semitone (notes) or a row (clips)",
            ),
            bind("Ctrl+\u{2190} \u{2192}", "Move a bar"),
            bind("Ctrl+\u{2191} \u{2193}", "Move an octave"),
            bind("Shift+\u{2190} \u{2192}", "Change the length"),
            bind("Shift+\u{2191} \u{2193}", "Nudge the property lane's value"),
            bind("Ctrl+Shift+arrows", "The same, by a bar / by ten"),
        ],
    },
    KeybindSection {
        title: "Mixer",
        binds: &[
            bind("M", "Mute the selected track"),
            bind("N", "Solo the selected track"),
        ],
    },
    KeybindSection {
        ...
        binds: &[
            ...
            bind("Enter", "Choose the row"),
            bind("Type", "Filter a menu or the plugin picker as you type"),
            bind(
    ...
    KeybindSection {
        ...
        binds: &[
            ...
            bind("Ctrl+C / Ctrl+X / Ctrl+V", "Copy, cut, paste"),
            bind("Home / End", "Start or end of the text"),
            bind("Shift+arrows", "Extend the selection"),
            bind("Ctrl+\u{2190} \u{2192}", "A word at a time"),
            bind("Enter", "Done"),
            bind("Esc", "Leave the field"),
        ],
    },
    KeybindSection {
        title: "Editor windows",
        binds: &[
            bind("Esc", "Close the window"),
            bind("Delete", "Remove the selected EQ band"),
            bind(
                "Space, Ctrl+Z, Ctrl+S…",
    ...
    KeybindSection {
        ...
        binds: &[
            ...
            bind("Alt", "Drag free of the grid"),
            bind(
                "Ctrl",
                "Marquee select with any tool; add a preset as a new channel",
            ),
            bind("Ctrl / Shift", "Preview a sound an octave down / up"),
            bind("Ctrl+wheel", "Zoom in time"),
            bind("Ctrl+Shift+wheel / Alt+wheel", "Zoom vertically"),
            bind("Shift+wheel", "Scroll sideways"),
        ],
    },
    KeybindSection {
        title: "Mouse: clicks",
        binds: &[
            bind("Right-click a note", "Delete it, whatever the tool"),
            bind("Right-click a control", "Automate it"),
            bind("Click the tempo", "Type a tempo; drag to slide it"),
            bind("Double-click the arrangement", "A new blank clip"),
            bind(
                "Shift+click the arrangement",
                "A copy of the last clip you picked",
            ),
            bind("Shift+drag a clip's right edge", "Make it loop"),
            bind("Double-click an audio clip", "Open its editor"),
            bind(
                "Click a sound",
                "Preview it; double-click to choose or import",
            ),
            bind("Ctrl+click an EQ band", "Remove the band"),
            bind("Ctrl+click a chooser", "Step it backwards"),
        ],
    },
];
...
pub enum KeybindRow {
    ...
    /// One binding: the key chip and, beside it, what it does.
    Bind {
        section: usize,
        index: usize,
        keys: Rect,
...
pub fn keybinds_layout(window: Rect, metrics: &Metrics, scroll: f32) -> KeybindsLayout {
    ...
    let title = Rect::new(
        inner.x,
        inner.y,
        (inner.width - row - KEYS_GAP).max(0.0),
        row,
    )
    .intersection(&inner)
    .clamped();
    ...
    for (index, section) in KEYBIND_SECTIONS.iter().enumerate() {
        ...
        y += heading;
        for bind_index in 0..section.binds.len() {
...
pub enum KeybindsHit {
    /// The × — close the sheet.
    Close,
    /// Somewhere on the card — nothing; the sheet stays.
    Card,
...
pub fn keybinds_hit(layout: &KeybindsLayout, x: f32, y: f32) -> KeybindsHit {
    if layout.close.contains(x, y) {
        KeybindsHit::Close
    } else if layout.frame.contains(x, y) {
        KeybindsHit::Card
    } else {
        KeybindsHit::Outside
    }
