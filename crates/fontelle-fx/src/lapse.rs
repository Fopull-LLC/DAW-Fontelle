//! Lapse: a few seconds of memory with curves drawn over it
//! (`docs/lapse-plan.md` §4).
//!
...
use fontelle_types::{
    LAPSE_MEMORY_SECONDS, LapseConfig, LapseGrid, LapseLaneKind, LapseNotes, LapseQuality,
    LapseSync, LapseTone, MusicalTime, lane_ticks,
};

use crate::NoteInput;

/// Channels one Lapse will keep a memory for.
...
pub const LAPSE_BUCKETS: usize = 512;
...
pub struct LapseFrame {
    ...
    pub clamped: bool,
    /// Seconds of memory actually written, up to [`LAPSE_MEMORY_SECONDS`].
    pub filled_seconds: f32,
}

impl Default for LapseFrame {
    fn default() -> Self {
        Self {
            phase: 0.0,
            offset: 0.0,
...
impl Lapse {
    ...
    pub fn prepare(&mut self, sample_rate: f32) {
        ...
        let frames = (LAPSE_MEMORY_SECONDS * self.sample_rate).ceil() as usize;
        ...
        self.buckets = vec![(0.0, 0.0); LAPSE_BUCKETS];
        self.bucket_samples = (frames / LAPSE_BUCKETS).max(1);
        self.reset();
    }
    ...
    pub fn reset(&mut self) {
        ...
        self.bucket_fill = 0;
        self.frame = LapseFrame::default();
    }

    /// What the window draws.
    pub fn frame(&self) -> LapseFrame {
        self.frame
    }
    ...
    pub fn process(
        ...
        let time_lane = grid.lane(scene, LapseLaneKind::Time);
        let volume_lane = grid.lane(scene, LapseLaneKind::Volume);
        let tone_lane = grid.lane(scene, LapseLaneKind::Tone);
        let pan_lane = grid.lane(scene, LapseLaneKind::Pan);
        ...
        let quality = match config.quality {
            LapseQuality::Normal => Interpolation::Normal,
            LapseQuality::High => Interpolation::High,
        };
        ...
        self.frame = LapseFrame {
            ...
            filled_seconds: (self.written as f32 / self.sample_rate).min(LAPSE_MEMORY_SECONDS),
        };
        if !music.rolling {
            self.idle_tick = tick;
        }
    ...
    fn scene_of(&mut self, notes: NoteInput, config: &LapseConfig, music: MusicalTime) -> usize {
        if config.notes != LapseNotes::Off {
            if notes.ons != self.last_ons {
                self.last_ons = notes.ons;
                if config.notes == LapseNotes::Retrigger {
    ...
    fn clock_tick(&mut self, config: &LapseConfig, music: MusicalTime) -> f64 {
        let song = self.song_tick(music);
        match config.sync {
            LapseSync::Song => song,
            LapseSync::Retrigger | LapseSync::Free => {
                if self.origin == 0.0 && song != 0.0 && config.sync == LapseSync::Free {
                    self.origin = song;
                }
                song - self.origin
            }
    ...
    fn set_tone(&mut self, config: &LapseConfig) {
        if self.tone_value == 0.0 {
            return;
        }
        ...
        let (mode, hz) = match config.tone_mode {
            LapseTone::LowHigh | LapseTone::Band => {
                if self.tone_value < 0.0 {
        ...
        if config.tone_mode == LapseTone::Band {
    ...
    fn filter(&mut self, index: usize, sample: f32, mode: LapseTone) -> f32 {
        let first = self.tone[index].process(sample, &self.tone_coeffs);
        if mode == LapseTone::Band {
...
fn read_ring(memory: &[f32], position: f64, written: u64, quality: Interpolation) -> f32 {
    ...
    // off. A fresh Lapse is a wire *sample for sample*, and a wire that moved
