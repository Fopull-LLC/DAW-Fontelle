...
automation system, not an independent data structure. **Built 2026-09-02:**
`fontelle_model::effective_tempo_map` is the document's own map bent by any clip aimed at
`transport/tempo`, sampled every sixteenth note into constant segments (a run of equal steps stays
one segment, so a flat lane costs one). Everything that turns a tick into a sample goes through
it — the compiler, the playhead, both rulers, a loop range — and `fontelle_types` publishes the
one mapping between a normalised lane value and a BPM (`TEMPO_MIN_BPM`..`TEMPO_MAX_BPM`, 20 to
300, deliberately narrower than the tempo box's own range so an ordinary song is not squashed
into the bottom of the block). A tempo lane emits **no** `ParamValue` events: it has no node to
send one to.
...
**Setting it is a right-drag on either ruler** (built 2026-09-02): the arrangement's or the piano
roll's, one gesture and one function (`canvas::time_selection`), snapped to that ruler's own grid.
A drag whose ends land on the same grid line is a click, and a click clears the selection — which
is what lets one button both make and unmake a loop. The roll works in the open clip's ticks and
the document stores the song's; the conversion is the document host's.

**Song and clip mode** (built 2026-09-02). FL's pattern mode: `PlayMode::Clip` compiles **one
clip** (`fontelle_sequencer::CompileScope`) and loops its own bars, so a part can be heard by
itself while it is being written; `PlayMode::Song` is the arrangement end to end and whatever
time selection is set. A clip scope leaves every other clip out, other lanes' automation and the
tempo lane included — a lane on another row is not part of the part being soloed — and the
window's own tick/sample conversions follow the same scope, or the playhead is drawn in a bar the
notes are not in.
...
every mixer track volume/pan/send level, and **tempo** (the last of these built 2026-09-02). The tempo map (§6.2) is generated by
...
- Right-click any knob or slider → "Create automation clip" places a clip on a lane of its own,
  pre-targeted. **Built 2026-09-02**, and three details of this line were decided against it in
  use, so what ships differs from what is written above:
  - **A lane of its own, not "the current lane".** The current lane is the one carrying the notes,
    and a curve drawn over them is one of the two hidden — reported as *"a lane of them looks like
    a lane of empty clips"*.
  - **Spanning the time selection, or the whole song** — not one bar at the playhead, which is a
    clip you have to stretch before you can draw anything worth hearing in it.
  - **Flat at the value the control is already at**, so making one changes nothing you can hear.
    A lane that jumped the parameter the moment it was created is a lane nobody trusts.
  - **A second gesture on the same control hands back the clip it already has.** Two clips over
    one target is §12.2's rule 1 applied to a curve somebody drew, which is a curve silently
    replaced.
- **The clip is edited where it sits**, inside its own block on the arrangement, FL-style: a
  caption band across the top that moves and selects the block, and under it the curve, where a
  click makes a point, a drag carries one and a right-click on one offers the six shapes and
  Delete. It is **not** a window — see §16.1.
...
- Draw modes: pencil (freehand, decimated), line, curve, and shape stamps (LFO shapes at a
  chosen rate/division, which is the fastest route to rhythmic automation). *Not built:* points
  are placed and dragged one at a time, and a marquee over several inside a block is still owed.
...
**What is a window and what is not (settled 2026-09-02).** Everything that edits one *device* is
an OS window, because that is the shape a third-party plugin editor will need: the instrument
(§7.2) and each insert (§13.4). Everything that is a view of the *document* is a panel: the notes,
the arrangement, the mixer. An **automation clip is a document view**, not a device — it was a
window for one pass and the window could not be made to do anything, which is the report §12.4
records. It is edited inside its block on the arrangement.
